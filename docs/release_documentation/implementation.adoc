...
//versteht man den vorletzten Satz?
Die Implementationsaufgaben bzw. -issues wurden hautpsächlich von Herrn Werner erstellt. Wohlwissend das es ein Risiko sein könnte, wenn lediglich eine Person dies erledigt. Jedoch hat er durch seine Ausbildung und weitreichendes Wissen über das Programm und die Use-Cases eine exzellente Grundlage um dies zu erfüllen. 
Die Reihenfolge der Issues wurde von den beschriebenen und priorisierten Use-Cases (UC) getrieben, welche in der aktuallisierten Anforderungsanalyse notiert sind. Jedoch haben diese Use-Cases gewisse Abhängigkeiten hinsichtlich der Nutzbarkeit in der Realität. So bringt es in der Praxis nichts, dass aus Use-Case 1 die Status in eine andere Woche kopiert werden können, wenn diese noch gar nicht durch den Benutzer hinzugefügt oder gar gelöscht werden könnten (UC 2). Daher wurden öfters neben "Kleinigkeiten" auch größere Aufgaben voran getrieben wurden, welche in den nächsten Iterationen große Funktionalitätsschritte ermöglichen. Die erfüllten Aufgaben sind mit "(#50)" zu GitHub verlinkt.
...
Es zeichnete sich ab, dass neben dem Fernbleiben von Herrn Keil und Frau Tartz auch einige andere Teammitglieder eine nur geringe Beteiligung vorweisen konnten. Daher drohte eine fallende Moral der anderen Mitgliedern. Dieser Umstand wurde im Teammeeting thematisiert und um Besserung gebeten. Aber aufgrund fehlender Dringlichkeit und da wir wussten nicht wussten, wie wir dieses Problem beheben können, haben wir von weiteren Maßnahmen abgesehen. 
...
//keine ahnung ob das hier gut ist oder eher unnötig 
//vllt sollte das auch eher in den fließtext eingearbeitet werden
Unsere vergebenen Rollen wurden lediglich zum Teil eingehalten. Herr Kemter erfüllte seine Rolle als Teamleiter, ebenso wie Herr Beck und Herr Nguyen in der Implementation. Jedoch wurde die Rolle des Architekten nicht mehr benötigt. Daher hätte Herr Werner und Herr Kirchhoff ebenfalls die Rolle der Implementatio erhalten können, ebenso wie Herr Tichonow. Jener hat sich im Verlaufe des Projektes nicht mit den Tests beschäftigt bzw. sich darum gekümmert.  
