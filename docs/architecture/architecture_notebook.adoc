...
Die logische Sicht wird durch ein C4 Modell dargestellt. Das C4-Modell ermöglicht es uns die verschieden Elemente und ihre Beziehungen darzustellen, damit kannn die Architekture unseres Programmes in verschieden Detailgraden dargestellt werden. 

Im Moment existieren nur 2 Ebenen, da die eine dritte schon die noch nicht Fertige und sich ändernte Implementierung illustrieren würde.
...
Momentan wird die Webanwendung für Test- und Vorzeigezwecke auf Docker der jeweiligen Gruppenmitglieder gehostet. Dies hat den Sinn, dass eine Änderung des Codes sofort im Webbrowser umsetzbar ist und eventuelle Fehler nicht die anderen Gruppenmitglieder aktiv stören. 
