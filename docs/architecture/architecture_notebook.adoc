= Architecture Notebook: E1_Ressourcenverwaltung
...
* Der Browser unterstützt HTML5-fähigen Browser
...
//NIKLAS HAT DAVON MEHR AHNUNG (FF.)
* Auf dem genutzten Server wurde ein Docker installiert. Dies hat den Grund, dass die erstellte Anwendung universell angewendet werden kann.

* Wir benutzen eine relationales Datenbankmodell (MariaDB auf dem Server installiert), da sich unsere Datenobjekte dafür anbieten.
...
* Um die Daten und Aktionen von dem Benutzer auf der Webseite in die Datenbank zu übertragen wird PHP genutzt. PHP ist einfach zu benutzen und kostenlos. 
...
* Status: enthält alle Informationen zu den Stati, zum Beispiel welcher Mitarbeiter welchen konkreten Projekten zugeordnet ist.

* Projekt: enthält konkrete Informationen zu den Projekten, zum Beispiel Name, Erstelldatum, Benutzer
...
Unsere logische Sicht beinhaltet die Ebenen, auf denen die Daten bearbeitet werden. Unsere Datenobjekte befinden sich im Hintergrund auf der Maria DB. Mittels PHP können sie an den Apache Server übertragen werden. Dabei vereinfacht der Docker die Bereitstellung der Anwendungen.
