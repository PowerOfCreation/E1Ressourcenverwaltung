...
Wöchentlich wird der Stand des Projektes in den Teammeetings abgeglichen. Dadurch hat konnte eine Übersicht der vergebenen und auch beendeten Aufgaben erfasst werden. Die Iterationslänge in diesem Projekt wurde auf eine Woche festgelegt, damit hatte jedes Mitglied eine zeitliche Vorgabe bis wann es seine Aufgabe im angemessen Maße bearbeitet haben sollte. 
Die Größe der Aufgaben werden in T-Shirt-Größen angegeben. 
Die Bewertung der Iterationen erfolgt in der Gruppe. 

//ziemlich sinnvoll und auch gut umzusetzen 
//gibt struktur und kann auch "rückblicken"
- Whole Team
...
//"whole team" nicht alle aspekte aber manche
